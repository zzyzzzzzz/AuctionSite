...
{
    //静态方法，负责启动数据库的初始化
    //它接收一个 WebApplication 对象（app），
    //这是 ASP.NET Core 应用程序的核心对象，用来处理应用程序中的各种服务。
    public static void InitDb(WebApplication app)
    {
        //使用了 app.Services.CreateScope() 来创建一个服务的 scope（范围）并确保获取的服务对象在此范围内有效。
        //当使用 using 语法时，会自动释放资源（在 scope 使用完后自动销毁）。
        using var scope = app.Services.CreateScope();


        //通过 scope.ServiceProvider.GetService<AuctionDbContext>() 获取数据库上下文 AuctionDbContext 实例
        SeedData(scope.ServiceProvider.GetService<AuctionDbContext>());
    }

    //接受一个数据库上下文（AuctionDbContext）
    //它负责检查数据库中是否存在数据，如果没有，则添加初始数据
    private static void SeedData(AuctionDbContext context)
    {
        context.Database.Migrate();//这是一个同步方法，会自动创建数据库和表
        //first check if we already have some auctions in the database
        ...
        {
            //auctions in courseasstes acutionsSeed.txt
